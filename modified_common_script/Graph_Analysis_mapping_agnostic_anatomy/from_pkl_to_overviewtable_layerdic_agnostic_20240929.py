...
def main():
    ...
    layerdictionary_direction = input('Please enter the layerdic directory: ')
    ...
    for i in filelist:
        ...
        slice_id = MAIN_RECORDING_DICTIONARY['Infos_Recording']['slice_id']
        
        
        
        
        try:
            os.chdir(layerdictionary_direction)
            layerlist = glob.glob('*.txt')
            layerfile = [file for file in layerlist if slice_id in file][0]
            
            with open(layerfile, 'r') as file:
                dict_str = file.read()

            layerdic = ast.literal_eval(dict_str)
        ...
        for region, channels in layerdic.items():
            for channel in channels:
                if channel in G.nodes():
                    G.nodes[channel]['region'] = region
        ...
        '''
        subgraph analysis depending on the regions
        
        '''
        
        for region in layerdic:
            subgraph = G.subgraph([node for node, data in G.nodes(data=True) if data.get('region') == region])
            ...
            try:
                ...
                if nx.is_connected(subgraph):
                    dgc = dict(nx.degree_centrality(subgraph))
                    mean_degree_centrality = np.round(np.mean(list(dgc.values())), 4)
                else:
                    logging.warning("Graph is disconnected. Skipping degree centrality calculations.")
            except Exception as e:
            ...
            try:
                # Closeness Centrality
                if nx.is_connected(subgraph):
                    cc = dict(nx.closeness_centrality(subgraph))
                    closeness_centrality = np.round(np.mean(list(cc.values())), 4)
                else:
                    logging.warning("Graph is disconnected. Skipping closeness centrality calculations.")
            except Exception as e:
            ...
            try:
                # Eigenvector Centrality
                evc = dict(nx.eigenvector_centrality(subgraph))
                eigenvector_centrality = np.round(np.mean(list(evc.values())), 4)
            except Exception as e:
                logging.error(f"Error calculating eigenvector centrality: {e}")
            ...
            try:
                # Non-randomness
                non_randomness = nx.non_randomness(subgraph)
            except Exception as e:
                logging.error(f"Error calculating non-randomness: {e}")
            ...
            try: 
                ...
                fig.savefig(os.path.join(outputdirectory, 'degree_distribution'+filename+'_'+region+'_.png'), 
                            dpi = 300, bbox_inches='tight')
                fig.savefig(os.path.join(outputdirectory, 'digree_distribution'+filename+'_'+region+'_.eps'), 
                            dpi = 300, bbox_inches='tight')
                #plt.show()
                plt.close(fig)
            except:
            ...
            try:
                if dgc != None:
                    ...
                    fig.savefig(os.path.join(outputdirectory, 'graph_degreenes_centrality'+filename+'_'+region+'_.png'), 
                                dpi = 300, bbox_inches='tight')
                    fig.savefig(os.path.join(outputdirectory, 'graph_degreenes_centrality'+filename+'_'+region+'_.eps'), 
                                dpi = 300, bbox_inches='tight')
            ...
            try:
                if evc != None:
                    ...
                    fig.savefig(os.path.join(outputdirectory, 'eigenvector_centrality'+filename+'_'+region+'_.png'), 
                                dpi = 300, bbox_inches='tight')
                    fig.savefig(os.path.join(outputdirectory, 'eigenvector_centrality'+filename+'_'+region+'_.eps'), 
                                dpi = 300, bbox_inches='tight')
            ...
            number_of_spikes_per_region = 0
            for channel in layerdic[region]:
                spikes_per_region = (len(spikedic_MAD[channel]))
                number_of_spikes_per_region += spikes_per_region
            regions_firingrate = number_of_spikes_per_region/timelengthrecording_s
        
            string_for_dic = str(key)
            MAIN_RECORDING_DICTIONARY['fr_'+string_for_dic] = regions_firingrate
            
            MDF_dic['fr_'+region] = regions_firingrate
            MDF_dic['mean_degree_centrality_'+region] = mean_degree_centrality
            MDF_dic['closeness_centrality_'+region] = closeness_centrality
            MDF_dic['mean_eigenvector_centrality_'+region] = eigenvector_centrality
            MDF_dic['average_shortest_path_length_'+region] = average_shortest_path
            MDF_dic['average_clustering_coefficient_'+region] = average_clustering_coefficient
            MDF_dic['non_randomness_'+region] = str(non_randomness)
            MDF_dic['sw_sigma_'+region] = small_world_sigma
            MDF_dic['sw_omega_'+region] = small_world_omega
            MDF_dic['diameter_'+region] = diameter
            MDF_dic['node_connectivity_'+region] = node_connectivity
