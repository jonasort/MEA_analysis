...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  9 13:19:47 2024

@author: jonas ort, md, msc
...
def filter_cutouts(data, threshold):
    """
    Filter out rows from the data where the absolute maximum value exceeds the threshold.
    
    Parameters:
    data (np.ndarray): 2D numpy array where each row represents a cutout
    threshold (float): The maximum allowed absolute value
    
    Returns:
    np.ndarray: Filtered data with rows removed that exceed the threshold
    """
    # Calculate the maximum absolute value for each row
    max_abs_values = np.abs(data).max(axis=1)
    ...
    for file in filelist:
        ...
        for i in signal_cuts:
            ...
            if stopping_point - starting_point > 10:    
                ...
                spikearray_seconds = list(spikedic_seconds.values()) #changed 20241004
