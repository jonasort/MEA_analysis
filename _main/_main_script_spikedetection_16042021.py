...
for file in filelist:
    ...
    for i in range (0, len(np_analog_for_filter)):
        ...
        crossings = detect_threshold_crossings(
            bandpassfilteredsignal, sampling_frequency, threshold, 
            dead_time=0.003
            )
        ...
        fig=plot_signal_and_spikes_from_bandpassfilteredsignal(
            bandpassfilteredsignal, spikes, threshold, 
        ...
        print(channellabel)
        #restingspikedic[channellabel] = crossings[crossings>2750000] # nur crossings, wenn keine stimuli sind
        fig2=plot_signal_and_spikes_from_bandpassfilteredsignal(
            bandpassfilteredsignal, spikes, threshold, 
            channellabel, fs, time_in_sec, tick=tick
            )
        ...
        # Check/Uncheck if wavecoutouts are needed
        if len(crossings)>0:
            cutouts = extract_waveforms(
                bandpassfilteredsignal, sampling_frequency, crossings, 
                pre, post
                )
    ...
    trigger_on, trigger_off, diff_An = find_triggers(
        np_analog_stream_1_data, tick
        )
