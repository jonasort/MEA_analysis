...
def main():
    ...
    for file in filelist:
        ...
        adjacency_df = pd.DataFrame(index=raw_signal_dic.keys(), columns=raw_signal_dic.keys())

        # Calculate the Pearson correlation for each pair of electrodes
        for key1, value1 in raw_signal_dic.items():
            for key2, value2 in raw_signal_dic.items():
                corr_coeff, _ = pearsonr(value1, value2)
                adjacency_df.loc[key1, key2] = corr_coeff
        
        # Convert DataFrame values to float type
        adjacency_df = adjacency_df.astype(float)
        
        # Plot using Matplotlib
        plt.imshow(adjacency_df, cmap='coolwarm', interpolation='nearest')
        plt.colorbar(label='Pearson Correlation Coefficient')
        plt.xticks(range(len(adjacency_df)), adjacency_df.columns, rotation=90)
        plt.yticks(range(len(adjacency_df)), adjacency_df.index)
        plt.show()
        ...
        #next: to the same for bandpassfiltered
        
        # do the same for lowpassfiltered
