...
# also join bandbpass and lowpass dictionaries for plotting
# lowpass
joined_lowpass_dic = {}
dd = defaultdict(list)
for timekey in  full_lowpass_dic:
    for key, value in full_lowpass_dic[timekey].items():
        for i in value:
            dd[key].append(i)
for key in dd:
    sort_values = sorted(dd[key])
    joined_lowpass_dic[key] = sort_values

# bandpass
joined_bandpass_dic = {}
dd = defaultdict(list)
for timekey in  full_bandpass_dic:
    for key, value in full_bandpass_dic[timekey].items():
        for i in value:
            dd[key].append(i)
for key in dd:
    sort_values = sorted(dd[key])
    joined_bandpass_dic[key] = sort_values
